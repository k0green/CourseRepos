using System;

namespace Lesson3_Matrix
{
    class Program
    {
        static int ReadSmth()
        {
            while (true)
            {
                if (int.TryParse(Console.ReadLine(), out int number))
                {
                    return number;
                }
            }
        }
        static int[,] CreateMatrix(in int AmOfStr, in int AmOfCol)
        {
            int[,] Matrix = new int[AmOfStr, AmOfStr];
            Console.WriteLine("Enter elements:");
            for (int i = 0; i < AmOfStr; i++)
            {
                for (int j = 0; j < AmOfCol; j++)
                {
                    Console.Write($"Matrix[{i + 1},{j + 1}]=");
                    Matrix[i, j] = Convert.ToInt32(Console.ReadLine());
                }
            }
            return Matrix;
        }
        static void ShowMatrix(ref int[,] Matrix, int AmountOfSting, int AmountOfColumn)
        {
            Console.WriteLine("Show elements:");
            for (int i = 0; i < AmountOfSting; i++)
            {
                for (int j = 0; j < AmountOfColumn; j++)
                    Console.Write($"{Matrix[i, j]} ");
                Console.Write("\n");
            }
        }
        static void FindNum(ref int[,] Matrix, int AmountOfSting, int AmountOfColumn)
        {
            int PositiveAmount = 0, NegativeAmount = 0; ;
            for (int i = 0; i < AmountOfSting; i++)
            {
                for (int j = 0; j < AmountOfColumn; j++)
                {
                    if (Matrix[i, j] > 0)
                    {
                        PositiveAmount++;
                    }
                    else if (Matrix[i, j] < 0)
                    {
                        NegativeAmount++;
                    }
                }
            }
            Console.WriteLine($"Number of positive numbers is {PositiveAmount}\n" +
                          $"Number of negstive numbers is {NegativeAmount}");
        }
        static void SortStringAscending(ref int[,] Matrix, int AmountOfSting, int AmountOfColumn, int NumOfString = 0)
        {
            NumOfString -= 1;
            int temp;
            for (int i = 0; i < AmountOfColumn - 1; i++)
            {
                for (int j = 0; j < AmountOfColumn - i - 1; j++)
                {
                    if (Matrix[NumOfString, j] > Matrix[NumOfString, j + 1])
                    {
                        temp = Matrix[NumOfString, j];
                        Matrix[NumOfString, j] = Matrix[NumOfString, j + 1];
                        Matrix[NumOfString, j + 1] = temp;
                    }
                }
            }

        }
        static void SortStringDescending(ref int[,] Matrix, int AmountOfSting, int AmountOfColumn, int NumOfString = 0)
        {
            NumOfString -= 1;
            int temp;
            for (int i = 0; i < AmountOfColumn - 1; i++)
            {
                for (int j = 0; j < AmountOfColumn - i - 1; j++)
                {
                    if (Matrix[NumOfString, j] < Matrix[NumOfString, j + 1])
                    {
                        temp = Matrix[NumOfString, j];
                        Matrix[NumOfString, j] = Matrix[NumOfString, j + 1];
                        Matrix[NumOfString, j + 1] = temp;
                    }
                }
            }

        }
        static void SortForStringInversion(ref int[,] Matrix, int AmountOfSting, int AmountOfColumn, int NumOfString = 0)
        {
            NumOfString -= 1;
            int temp;
            for (int i = 0; i < AmountOfColumn / 2; i++)
            {
                temp = Matrix[NumOfString, i];
                Matrix[NumOfString, i] = Matrix[NumOfString, AmountOfColumn - i - 1];
                Matrix[NumOfString, AmountOfColumn - i - 1] = temp;
            }
        }

        static void Main(string[] args)
        {
            Console.WriteLine("Enter amount of string:");
            int AmountOfSting = ReadSmth();
            Console.WriteLine("Enter amount of column:");
            int AmountOfColumn = ReadSmth();
            int[,] Matrix = new int[AmountOfSting, AmountOfColumn];
            Matrix = CreateMatrix(AmountOfSting, AmountOfColumn);
            ShowMatrix(ref Matrix, AmountOfSting, AmountOfColumn);

            bool check = false;
            do
            {
                Console.WriteLine("Do you want to do some features?");
                Console.WriteLine("Enter:\n1-Find num of positive and negative nubers;\n" +
                    "2-Sort string;");
                int NumOfFeature = ReadSmth();
                switch (NumOfFeature)
                {
                    case 1:
                        FindNum(ref Matrix, AmountOfSting, AmountOfColumn);
                        break;
                    case 2:

                        Console.WriteLine("Enter:\n1-Sort string ascending;\n2-Sort string descending;\n" +
                            "3-Sort for string inversion");
                        int NumOfTask = Convert.ToInt32(Console.ReadLine());
                        Console.WriteLine("To do this operatoin you need to enter number of string," +
                            " which u want to sort ");
                        int NumOfString = ReadSmth();
                        switch (NumOfTask)
                        {
                            case 1:
                                SortStringAscending(ref Matrix, AmountOfSting, AmountOfColumn, NumOfString);
                                break;
                            case 2:
                                SortStringDescending(ref Matrix, AmountOfSting, AmountOfColumn, NumOfString);
                                break;
                            case 3:
                                SortForStringInversion(ref Matrix, AmountOfSting, AmountOfColumn, NumOfString);
                                break;
                        }
                        break;
                }
                ShowMatrix(ref Matrix, AmountOfSting, AmountOfColumn);
                Console.WriteLine("Do you want to continue(yes/no)?");
                string answer = Console.ReadLine();
                if (answer == "yes")
                {
                    check = true;
                }
                else
                {
                    check = false;
                }
            } while (check == true);
            Console.ReadKey();
        }
    }
}
